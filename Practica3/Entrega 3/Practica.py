...
"""
    ...
    Ejercicio 3: Mosaico (N=2)
        
    #########################################################################
""" 

# Ejercicio 3
def Ejercicio3 (nombres_imagenes):
    
    # Se leen las imagen
    imagen1 = LeeImagenCV(nombres_imagenes[0],1)
    imagen2 = LeeImagenCV(nombres_imagenes[1],1)
    
    # Creamos la imagen donde se creará el mosaico
    mosaico_base = np.zeros((int(imagen1.shape[0]*3.5), int(imagen1.shape[1]*1.5), 3), dtype=np.uint8)
    ...
    # Encontramos la homografía entre las dos imágenes
    homografias, mask =  cv2.findHomography(puntos1, puntos2, cv2.RANSAC)
    
    # Montamos la primera imagen del mosaico
    mosaico = cv2.warpPerspective(src=imagen1, dst=mosaico_base, M=homografias, dsize=(mosaico_base.shape[0], mosaico_base.shape[1]), borderMode=cv2.BORDER_TRANSPARENT) 
    ...
    matches = Correspondecia (descriptor1, descriptor2, False)
    ...
    # Adaptamos los puntos a la homografía
    puntos1.reshape(-1,1,2)
    puntos2.reshape(-1,1,2)
    
    homografias, mask = cv2.findHomography(puntos1, puntos2, cv2.RANSAC)
    
    mosaico = cv2.warpPerspective(src=imagen2, dst=mosaico, M=homografias, dsize=(mosaico_base.shape[0], mosaico_base.shape[1]), borderMode=cv2.BORDER_TRANSPARENT)
    
    # Cuando tenemos el mosaico montado, lo pintamos
    img = AjustaTribanda(mosaico)
                
    img = NormalizarIntervalo(mosaico)
...
# Ejercicio 4
def Ejercicio4 (nombres_imagenes):
    
    # Se leen las imagen
    imagen_centro = LeeImagenCV(nombres_imagenes[0],1)
        
    # Creamos la imagen donde se creará el mosaico
    mosaico_base = np.zeros((int(imagen_centro.shape[0]*5), int(imagen_centro.shape[1]*2.5), 3), dtype=np.uint8)
    
    x = int(imagen_centro.shape[0] - imagen_centro.shape[0]/2)
    y = int(imagen_centro.shape[1] - imagen_centro.shape[1]/2)
    
    keypoints1, descriptor1, keypoints2, descriptor2 = Akaze(imagen_centro, mosaico_base)
   ...
   # Montamos la primera imagen del mosaico
    mosaico = cv2.warpPerspective(src=imagen_centro, dst=mosaico_base, M=homografias, dsize=(mosaico_base.shape[0], mosaico_base.shape[1]), borderMode=cv2.BORDER_TRANSPARENT) 
        
    
    # Completamos el mosaico con el resto de las imágenes
    for ind in range(1,len(nombres_imagenes)): 
        
        imagen = LeeImagenCV(nombres_imagenes[ind],1)
        
        
        # Sacamos la correspondencia de la imagen que queremos añadir y el mosaico actual
        keypoints1, descriptor1, keypoints2, descriptor2 = Akaze(mosaico, imagen)
        
        matches = Correspondecia (descriptor1, descriptor2, False)
        
        # Puntos seleccionados por medio de Lowe's ratio distance
        matches = LoweRatio (matches, 0.7)
        ...
        mosaico = cv2.warpPerspective(src=imagen, dst=mosaico, M=homografias, dsize=(mosaico_base.shape[0], mosaico_base.shape[1]), borderMode=cv2.BORDER_TRANSPARENT)
...
def main():
    ...
    print ("\n EJERCICIO 3: Mosaicos (N=2) ")
    
    print (" -> Ficheros: %s - %s" % (nombres_imagen[0], nombres_imagen[1]))
    Ejercicio3 (nombres_imagen)
    
    input("\n ENTER para continuar con el siguiente mosaico: ")
    nombres_imagen = ["imagenes/mosaico002.jpg","imagenes/mosaico006.jpg"]
...
#    Ejercicio3 (nombres_imagen)
    ...
    nombres_imagen = ["imagenes/mosaico002.jpg","imagenes/mosaico003.jpg",
                      "imagenes/mosaico004.jpg","imagenes/mosaico005.jpg",
                      "imagenes/mosaico006.jpg","imagenes/mosaico007.jpg",
                      "imagenes/mosaico008.jpg","imagenes/mosaico009.jpg",
                      "imagenes/mosaico010.jpg","imagenes/mosaico011.jpg"]
    
    print ("\n EJERCICIO 3: Mosaicos (N=10) ")
    
    print (" -> Mosaico: ")
    Ejercicio4 (nombres_imagen)   
